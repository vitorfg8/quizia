package com.vitorfg8.quizia.designsystem.icons

import androidx.compose.ui.graphics.vector.ImageVector

private var arrowBackRegular: ImageVector? = null

internal val ArrowBackRegular: ImageVector
    get() {
        val current: ImageVector? = arrowBackRegular
        if (current != null) {
            return current
        }
        return buildPhosphorIcon(
            name = "ArrowBack",
            autoMirror = true,
            paths = listOf(
                PhosphorPath(
                    data =
                        "M224,128a8,8,0,0,1-8,8H59.31l58.35,58.34a8,8,0,0,1-11.32,11.32l-72-72a8,8,0,0,1," +
                        "0-11.32l72-72a8,8,0,0,1,11.32,11.32L59.31,120H216A8,8,0,0,1,224,128Z",
                ),
            ),
        ).also { arrowBackRegular = it }
    }
